...
export function sendNotification(body, actions = []) {
	...
	const options = {
		...
		body: body,
		tag: "🏳️‍⚧️",
		icon: "./src/img/homeMade/icons/pwa/manifest-icon-192.maskable.png",
		actions: actions,
	};
	...
	if (globalThis.mvc.electron) globalThis.mvc.electron.sendNotification({
		...
		globalThis.mvc.capacitor.sendNotification(body, title);
	}
...
export function assertInstance(obj, type) {
	if (!(obj instanceof type)) throw new ArchitectureError(`${obj} is not an instance of ${type}`);
	return obj;
}